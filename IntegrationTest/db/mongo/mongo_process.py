...
class MongoProcess(DbProcess):

    def __init__(self, dbname, host="127.0.0.1", port=27017):
        DbProcess.__init__(self, dbname, host, port)
    ...
    def init(self):
        ...
        config = [{
            ...
            "param_key": "SMS_HOST",
            "param_value": "http://127.0.0.1:3008/"
        }, {
        ...
        seq_info.insert_one({"tab_name": "uiac_user", "sid": 10000000})
        ...
        user.create_index([("account", ASCENDING)])
    ...
    def set_expire_time(self, expire_time):
        db = self.client.get_database(self.dbname)
        app_setting = db.get_collection("app_setting")
        app_setting.update({"param_key": "ACCESS_TOKEN_EXPIRE_TIME"}, {"$set": {"param_value": expire_time}})

    def clear(self):
        self.client.drop_database(self.dbname)

# if __name__ == "__main__":
	# mongo = MongoProcess("uiac", "192.168.9.16", 27017)
    # mongo.init()
    # mongo.set_expire_time("100")
    # mongo.clear()
